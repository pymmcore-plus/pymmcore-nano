...
np_array make_np_array_from_copy(const void *src,
                                 size_t nbytes,
    ...
    nb::capsule owner(raw_ptr, [](void *ptr) noexcept { delete[] static_cast<uint8_t *>(ptr); });
 ...
 *
 */
np_array build_grayscale_np_array(CMMCore &core,
                                  void *pBuf,
                                  unsigned width,
                                  unsigned height,
                                  unsigned byteDepth) {
...
np_array build_rgb_np_array(CMMCore &core,
                            void *pBuf,
                            unsigned width,
                            unsigned height,
                            unsigned byteDepth) {
...
void validate_slm_image(const nb::ndarray<uint8_t> &pixels,
                        long expectedWidth,
                        long expectedHeight,
                        long bytesPerPixel) {
    // Check dtype
...
class PyMMEventCallback : public MMEventCallback {
  public:
    ...
    void
    onPropertyChanged(const char *name, const char *propName, const char *propValue) override {
    ...
    void onPixelSizeAffineChanged(double v0,
                                  double v1,
                                  double v2,
                                  double v3,
                                  double v4,
...
NB_MODULE(_pymmcore_nano, m) {
    ...
    m.attr("MMCore_version_info") = std::tuple(CMMCore::getMMCoreVersionMajor(),
                                               CMMCore::getMMCoreVersionMinor(),
...
#define BIND_ENUM_VALUE(enum_obj, name, enum_value)                                              \
    ...
    BIND_ENUM_VALUE(device_type_enum,
    ...
    // PropertyType enum
    auto property_type_enum = nb::enum_<MM::PropertyType>(m, "PropertyType", nb::is_arithmetic());
    ...
    BIND_ENUM_VALUE(focus_direction_enum,
    ...
    BIND_ENUM_VALUE(device_notification_enum,
                    "StatusChanged",
    ...
    BIND_ENUM_VALUE(device_detection_status_enum,
                    "Unimplemented",
    ...
    auto device_initialization_state_enum =
        nb::enum_<DeviceInitializationState>(m, "DeviceInitializationState", nb::is_arithmetic());
    BIND_ENUM_VALUE(device_initialization_state_enum,
                    "Uninitialized",
...
A configuration is a collection of device property settings.
    ...
    nb::class_<PropertySetting>(m, "PropertySetting")
        ...
        .def("isEqualTo",
             &PropertySetting::isEqualTo,
             "other"_a,
             "Checks if this property setting is equal to another")
        .def_static("generateKey",
                    &PropertySetting::generateKey,
                    "device"_a,
                    "prop"_a,
    ...
    nb::class_<MetadataTag>(m, "MetadataTag")
        ...
        .def("Restore",
             nb::overload_cast<const char *>(&MetadataTag::Restore),
             "stream"_a,
    ...
    nb::class_<MetadataSingleTag, MetadataTag>(m, "MetadataSingleTag")
        .def(nb::init<>(), "Default constructor")
        .def(nb::init<const char *, const char *, bool>(),
             "name"_a,
             "device"_a,
             "readOnly"_a,
...
Use by passing an instance to [`CMMCore.registerCallback`][pymmcore_nano.CMMCore.registerCallback].
        ...
        .def("onPropertyChanged",
             &MMEventCallback::onPropertyChanged,
             "name"_a,
             "propName"_a,
             "propValue"_a,
             "Called when a specific property is changed")
        .def("onChannelGroupChanged",
             &MMEventCallback::onChannelGroupChanged,
             "newChannelGroupName"_a,
             "Called when the channel group changes")
        .def("onConfigGroupChanged",
             &MMEventCallback::onConfigGroupChanged,
             "groupName"_a,
             "newConfigName"_a,
        ...
        .def("onPixelSizeChanged",
             &MMEventCallback::onPixelSizeChanged,
             "newPixelSizeUm"_a,
             "Called when the pixel size changes")
        .def("onPixelSizeAffineChanged",
             &MMEventCallback::onPixelSizeAffineChanged,
             "v0"_a,
             "v1"_a,
             "v2"_a,
             "v3"_a,
...
programming.
            ...
            .def(
                ...
                &CMMCore::saveSystemConfiguration,
                "fileName"_a,
            ...
            .def("loadDevice",
                 ...
                 "moduleName"_a,
                 "deviceName"_a,
                 "Loads a device from the plugin library." RGIL)
            .def("unloadDevice",
                 &CMMCore::unloadDevice,
                 "label"_a,
            ...
            .def("getDeviceInitializationState",
                 &CMMCore::getDeviceInitializationState,
                 "label"_a,
            ...
            .def("getCoreErrorText",
                 &CMMCore::getCoreErrorText,
                 "code"_a,
            ...
            .def(
                "loadSystemState",
                &CMMCore::loadSystemState,
                "fileName"_a,
                "Loads the system configuration from the text file conforming to the MM specific format. The configuration contains a list of commands to build the desired system state from read-write properties." RGIL)
            .def("registerCallback",
                 &CMMCore::registerCallback,
...
MMCore will send notifications on internal events using this interface
            ...
            .def("getPrimaryLogFile",
                 &CMMCore::getPrimaryLogFile,
                 "Return the name of the primary Core log file." RGIL)
            .def("logMessage",
                 nb::overload_cast<const char *>(&CMMCore::logMessage),
                 "msg"_a RGIL)
            .def("logMessage",
            ...
            .def(
                ...
                [](CMMCore &self,
                   ...
                   bool truncate,
                   bool synchronous) {
                    return self.startSecondaryLogFile(nb::str(filename).c_str(),
                                                      enableDebug,
                                                      truncate,
                                                      synchronous);
                },
            ...
            .def("stopSecondaryLogFile",
                 &CMMCore::stopSecondaryLogFile,
                 "handle"_a,
                 "Stop capturing logging output into an additional file." RGIL)

            .def("getDeviceAdapterSearchPaths",
                 &CMMCore::getDeviceAdapterSearchPaths,
                 "Return the current device adapter search paths." RGIL)
            .def("setDeviceAdapterSearchPaths",
                 &CMMCore::setDeviceAdapterSearchPaths,
                 "paths"_a,
                 "Set the device adapter search paths." RGIL)
            .def("getDeviceAdapterNames",
                 &CMMCore::getDeviceAdapterNames,
                 "Return the names of discoverable device adapters." RGIL)
            .def("getAvailableDevices",
                 &CMMCore::getAvailableDevices,
                 "library"_a,
            ...
            .def("getAvailableDeviceTypes",
                 &CMMCore::getAvailableDeviceTypes,
                 "library"_a,
            ...
            .def("getLoadedDevicesOfType",
                 &CMMCore::getLoadedDevicesOfType,
                 "devType"_a,
            ...
            .def("getDevicePropertyNames",
                 &CMMCore::getDevicePropertyNames,
                 "label"_a,
            ...
            .def(
                "systemBusy",
                &CMMCore::systemBusy,
                "Checks the busy status of the entire system. The system will report busy if any of the devices is busy." RGIL)
            .def("waitForSystem",
                 &CMMCore::waitForSystem,
                 "Blocks until all devices in the system become ready (not-busy)." RGIL)
            .def(
                "deviceTypeBusy",
                &CMMCore::deviceTypeBusy,
                "devType"_a,
            ...
            .def("waitForDeviceType",
                 &CMMCore::waitForDeviceType,
                 "devType"_a,
                 "Blocks until all devices of the specific type become ready (not-busy)." RGIL)
            ...
            .def("sleep",
                 &CMMCore::sleep,
                 "intervalMs"_a,
                 "Waits (blocks the calling thread) for specified time in milliseconds." RGIL)

            .def("getCameraDevice",
                 &CMMCore::getCameraDevice,
                 "Returns the label of the currently selected camera device." RGIL)
            .def("getShutterDevice",
                 &CMMCore::getShutterDevice,
            ...
            .def("updateSystemStateCache",
                 &CMMCore::updateSystemStateCache,
            ...
            .def(
                ...
                &CMMCore::getCurrentConfigFromCache,
                "groupName"_a,
            ...
            .def(
                ...
                &CMMCore::getConfigGroupStateFromCache,
                "group"_a,
            ...
            .def("defineConfig",
                 ...
                 "propName"_a,
                 "value"_a RGIL)
            .def("defineConfigGroup",
                 &CMMCore::defineConfigGroup,
                 "groupName"_a,
                 "Creates an empty configuration group." RGIL)
            .def("deleteConfigGroup",
                 &CMMCore::deleteConfigGroup,
                 "groupName"_a,
            ...
            .def("isGroupDefined",
                 &CMMCore::isGroupDefined,
                 "groupName"_a,
                 "Checks if the group already exists." RGIL)
            .def("isConfigDefined",
                 &CMMCore::isConfigDefined,
                 "groupName"_a,
                 "configName"_a,
            ...
            .def("getPixelSizeUm",
                 nb::overload_cast<bool>(&CMMCore::getPixelSizeUm),
                 "cached"_a RGIL)
            .def("getPixelSizeUmByID",
                 &CMMCore::getPixelSizeUmByID,
                 "resolutionID"_a,
            ...
            .def(
                ...
                &CMMCore::getPixelSizeAffineByID,
                "resolutionID"_a,
            ...
            .def("getPixelSizedxdz",
                 nb::overload_cast<bool>(&CMMCore::getPixelSizedxdz),
                 "cached"_a RGIL)
            .def("getPixelSizedxdz",
                 nb::overload_cast<const char *>(&CMMCore::getPixelSizedxdz),
                 "resolutionID"_a RGIL)
            .def("getPixelSizedydz", nb::overload_cast<>(&CMMCore::getPixelSizedydz) RGIL)
            .def("getPixelSizedydz",
                 nb::overload_cast<bool>(&CMMCore::getPixelSizedydz),
                 "cached"_a RGIL)
            .def("getPixelSizedydz",
                 nb::overload_cast<const char *>(&CMMCore::getPixelSizedydz),
                 "resolutionID"_a RGIL)
            .def("getPixelSizeOptimalZUm",
                 nb::overload_cast<>(&CMMCore::getPixelSizeOptimalZUm) RGIL)
            .def("getPixelSizeOptimalZUm",
                 nb::overload_cast<bool>(&CMMCore::getPixelSizeOptimalZUm),
                 "cached"_a RGIL)
            .def("getPixelSizeOptimalZUm",
            ...
            .def(
                ...
                &CMMCore::setPixelSizeOptimalZUm,
                "resolutionID"_a,
                "optimalZ"_a,
                "Sets the opimal Z stepSize (in microns). There is no magic here, this number is provided by the person configuring the microscope, to be used by the person using the microscope." RGIL)
            ...
            .def("getPixelSizeConfigData",
                 &CMMCore::getPixelSizeConfigData,
                 "configName"_a,
            ...
            .def("isMultiROISupported",
                 &CMMCore::isMultiROISupported,
                 "Queries the camera to determine if it supports multiple ROIs." RGIL)
            .def("isMultiROIEnabled",
                 &CMMCore::isMultiROIEnabled,
            ...
            .def("getExposure",
                 nb::overload_cast<const char *>(&CMMCore::getExposure),
                 "label"_a RGIL)
            .def(
                "snapImage",
                &CMMCore::snapImage,
            ...
            .def("getImageWidth",
                 &CMMCore::getImageWidth,
                 "Horizontal dimension of the image buffer in pixels." RGIL)
            .def("getImageHeight",
                 &CMMCore::getImageHeight,
                 "Vertical dimension of the image buffer in pixels." RGIL)
            .def(
                "getBytesPerPixel",
                &CMMCore::getBytesPerPixel,
            ...
            .def(
                ...
                nb::overload_cast<const char *, long, double, bool>(&CMMCore::
                                                                        startSequenceAcquisition),
                "cameraLabel"_a,
                "numImages"_a,
                "intervalMs"_a,
            ...
            .def(
                ...
                &CMMCore::prepareSequenceAcquisition,
                "cameraLabel"_a,
            ...
            .def("getRemainingImageCount",
                 &CMMCore::getRemainingImageCount,
                 "Returns number ofimages available in the Circular Buffer" RGIL)
            .def("getBufferTotalCapacity",
            ...
            .def("incrementalFocus",
                 &CMMCore::incrementalFocus,
                 "Performs incremental focus for the one-shot focusing device." RGIL)
            .def("setAutoFocusOffset",
                 &CMMCore::setAutoFocusOffset,
                 "offset"_a,
                 "Applies offset the one-shot focusing device." RGIL)
            .def("getAutoFocusOffset",
                 &CMMCore::getAutoFocusOffset,
            ...
            .def(
                ...
                "stateDeviceLabel"_a,
                "state"_a,
                "Sets the state (position) on the specific device. The command will fail if the device does not support states." RGIL)
            .def(
                "getState",
                &CMMCore::getState,
                "stateDeviceLabel"_a,
                "Returns the current state (position) on the specific device. The command will fail if the device does not support states." RGIL)
            .def("getNumberOfStates",
                 &CMMCore::getNumberOfStates,
                 "stateDeviceLabel"_a,
                 "Returns the total number of available positions (states)." RGIL)
            .def("setStateLabel",
                 &CMMCore::setStateLabel,
                 "stateDeviceLabel"_a,
                 "stateLabel"_a,
                 "Sets device state using the previously assigned label (string)." RGIL)
            .def("getStateLabel",
                 &CMMCore::getStateLabel,
                 "stateDeviceLabel"_a,
                 "Returns the current state as the label (string)." RGIL)
            ...
            .def("getStateFromLabel",
                 ...
                 "stateDeviceLabel"_a,
                 "stateLabel"_a,
                 "Obtain the state for a given label." RGIL)
            ...
            .def("setPosition",
                 ...
                 "position"_a RGIL)
            .def("setPosition",
                 nb::overload_cast<double>(&CMMCore::setPosition),
                 "position"_a RGIL)
            .def("getPosition",
            ...
            .def("setRelativePosition",
                 ...
                 "d"_a RGIL)
            .def("setOrigin",
                 nb::overload_cast<const char *>(&CMMCore::setOrigin),
                 "stageLabel"_a RGIL)
            ...
            .def(
                ...
                &CMMCore::getStageSequenceMaxLength,
                "stageLabel"_a,
                "Gets the maximum length of a stage's position sequence. This should only be called for stages that are sequenceable" RGIL)
            .def(
                "loadStageSequence",
            ...
            .def(
                ...
                "dZ_um"_a,
                "nSlices"_a,
                "Loads a linear sequence (defined by stepsize and nr. of steps) into the device. Why was it not called loadStageLinearSequence???" RGIL)
            ...
            .def("stop",
                 &CMMCore::stop,
                 "xyOrZStageLabel"_a,
                 "Stop the XY or focus/Z stage motors" RGIL)
            .def("home",
                 &CMMCore::home,
                 "xyOrZStageLabel"_a,
                 "Perform a hardware homing operation for an XY or focus/Z stage." RGIL)
            .def("setOriginXY",
            ...
            .def("setSLMPixelsTo",
                 ...
                 "blue"_a RGIL)
            .def("displaySLMImage",
                 &CMMCore::displaySLMImage,
                 "slmLabel"_a,
                 "Display the waiting image on the SLM." RGIL)
            .def(
                "setSLMExposure",
                &CMMCore::setSLMExposure,
                "slmLabel"_a,
                "exposure_ms"_a,
            ...
            .def("getSLMExposure",
                 &CMMCore::getSLMExposure,
                 "slmLabel"_a,
                 "Returns the exposure time that will be used by the SLM for illumination" RGIL)
            .def("getSLMWidth",
                 &CMMCore::getSLMWidth,
                 "slmLabel"_a,
                 "Returns the width (in \"pixels\") of the SLM" RGIL)
            .def("getSLMHeight",
                 &CMMCore::getSLMHeight,
                 "slmLabel"_a,
                 "Returns the height (in \"pixels\") of the SLM" RGIL)
            .def(
                "getSLMNumberOfComponents",
                &CMMCore::getSLMNumberOfComponents,
                "slmLabel"_a,
                "Returns the number of components (usually these depict colors) of the SLM For instance, an RGB projector will return 3, but a grey scale SLM returns 1" RGIL)
            .def("getSLMBytesPerPixel",
                 &CMMCore::getSLMBytesPerPixel,
                 "slmLabel"_a,
                 "Returns the number of bytes per SLM pixel" RGIL)
            // SLM Sequence
            .def(
                "getSLMSequenceMaxLength",
                &CMMCore::getSLMSequenceMaxLength,
                "slmLabel"_a,
                "For SLMs that support sequences, returns the maximum length of the sequence that can be uploaded to the device" RGIL)
            .def("startSLMSequence",
                 &CMMCore::startSLMSequence,
                 "slmLabel"_a,
                 "Starts the sequence previously uploaded to the SLM" RGIL)
            .def("stopSLMSequence",
                 &CMMCore::stopSLMSequence,
                 "slmLabel"_a,
            ...
            .def("getGalvoXRange",
                 &CMMCore::getGalvoXRange,
                 "galvoLabel"_a,
                 "Get the Galvo x range" RGIL)
            .def("getGalvoXMinimum",
                 &CMMCore::getGalvoXMinimum,
                 "galvoLabel"_a,
                 "Get the Galvo x minimum" RGIL)
            .def("getGalvoYRange",
                 &CMMCore::getGalvoYRange,
                 "galvoLabel"_a,
                 "Get the Galvo y range" RGIL)
            .def("getGalvoYMinimum",
                 &CMMCore::getGalvoYMinimum,
                 "galvoLabel"_a,
                 "Get the Galvo y minimum" RGIL)
            ...
            .def("deleteGalvoPolygons",
                 &CMMCore::deleteGalvoPolygons,
                 "galvoLabel"_a,
                 "Remove all added polygons" RGIL)
            .def("loadGalvoPolygons",
                 &CMMCore::loadGalvoPolygons,
                 "galvoLabel"_a,
            ...
            .def("pumpDispenseDurationSeconds",
                 &CMMCore::pumpDispenseDurationSeconds,
                 "pumpLabel"_a,
                 "seconds"_a,
                 "Dispenses for the provided duration (in seconds) at the set flowrate" RGIL)
            .def("pumpDispenseVolumeUl",
                 &CMMCore::pumpDispenseVolumeUl,
                 "pumpLabel"_a,
                 "microLiter"_a,
            ...
            .def(
                ...
                &CMMCore::supportsDeviceDetection,
                "deviceLabel"_a,
                "Return whether or not the device supports automatic device detection (i.e. whether or not detectDevice() may be safely called)." RGIL)
            .def(
                "detectDevice",
                &CMMCore::detectDevice,
                "deviceLabel"_a,
            ...
            .def("setParentLabel",
                 &CMMCore::setParentLabel,
                 "deviceLabel"_a,
                 "parentHubLabel"_a,
                 "Sets parent device label" RGIL)
            .def(
                "getInstalledDevices",
            ...
            .def("getLoadedPeripheralDevices",
                 &CMMCore::getLoadedPeripheralDevices,
                 "hubLabel"_a RGIL)
